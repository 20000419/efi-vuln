__int64 __fastcall GetDimensions(_BYTE *Image)
{
  ...
  const unsigned int MAX_HUFFMAN_TABLES = 4; // Match static array size
...
LABEL_26:
  ...
  {
    // Validate Huffman table count before storing
    if (HuffmanTablesCounter >= MAX_HUFFMAN_TABLES)
      return 0i64; // Reject too many Huffman tables
    
    v6 = HuffmanTablesCounter; // Use current count before increment
    HuffmanTables[v6] = (Image + 4);
    HuffmanTablesCounter++; // Increment after valid check
    goto LABEL_26;
  }
  ...
  if ( MarkerType != 0xDA )
  {
    if ( MarkerType == 0xDB )
    {
      if (v4 >= MAX_HUFFMAN_TABLES) // Optional additional check for other counters if needed
        return 0i64;
      v8 = v4++;
...
LABEL_30:
  ...
  if ( HuffmanTablesCounter == 1 )
  {
    ...
    do
    {
      // Ensure we don't exceed the valid table count during processing
      if (HuffmanTablesCounter - 1 >= MAX_HUFFMAN_TABLES)
        return 0i64;
  ...
  // Final validation of table count against expected maximum
