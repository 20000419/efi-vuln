__int64 __fastcall DecodeLZW(
        ...
        unsigned __int16 Height, // Renamed a6 to Height for clarity
        EFI_GRAPHICS_OUTPUT_BLT_PIXEL *BltOutput)
{
  ...
  {
    v21 = Height; // Use meaningful variable name
    v22 = qword_1D50;
    do
    {
      v23 = v16 >> 3;
      ...
      if (CodeVal >= Idx)
      {
        ...
        if (v13 != -1)
        {
          ...
          v15 = v33;
          v21 = Height; // Update with correct height variable
        }
        *v22 = v22[v26];
      }
      else
...
LABEL_23:
      if (v26 >= 0)
      {
        do
        {
          ...
          v31 = v22[XVar];
          LODWORD(XVar) = v17++;
          // Fixed OOB Write: Check if index is within valid bounds
          unsigned int PixelIndex = XVar + Width * YVar;
          if (PixelIndex >= Width * Height) {
            return -1; // Out of bounds access, return error
          }
          BltOutput[PixelIndex] = *(a4 + 4 * v31);
          if (v17 == Width)
          {
            if (a3)
            {
