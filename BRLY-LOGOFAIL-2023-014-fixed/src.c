unsigned __int8 *GetImageSize()
{
  ...
  if ( result )
  {
    ...
    do
    {
      ...
      if (Length > 0x1000000) // Arbitrary reasonable maximum size
        return NULL;
        
      if (ImageSize + Length + 12 < ImageSize) // Check for overflow
        return NULL;
        
      if (ImagePtrCursor + 4 + Length + 8 < ImagePtrCursor) // Check for overflow
        return NULL;
