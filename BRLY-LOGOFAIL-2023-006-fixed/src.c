        ...
        __int64 a7)
{
  ...
  if (Total >> 3 <= ImageSize)
  {
    ...
    do
    {
      ...
      // Added validation for CodeVal to prevent OOB access
      CodeVal = ((1 << v15) - 1) & (*&Image1[v23] >> v24);
      if (CodeVal >= Idx) // Invalid code - exceeds current dictionary size
        return -1;       // Return error code for invalid code
      ...
      if (v18 == v21)
        return v16;
      v26 = -1;
      if (CodeVal >= Idx) // This condition should now be unreachable due to earlier check
      {
        if (v13 == Bound)
          goto LABEL_44;
        v26 = 0;
      ...
      else
      {
        ...
        if (CodeVal != 0xFFFF)
        {
          // Added validation for initial code index
          if (v27 >= Bound) // Ensure initial code is within allocated heap entries
            return -1;     // Invalid initial code
      ...
      // Ensure we don't exceed heap array bounds during dictionary expansion
      if (Idx0 >= MAX_HEAP_ENTRIES) // Assume MAX_HEAP_ENTRIES is the allocated size
        return -1;                 // Prevent OOB write
